...
async function waitForLogto() {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    ...
    console.log(
      `Logto not ready, retrying in ${RETRY_DELAY / 1000}s... (${i + 1}/${MAX_RETRIES})`,
    );
...
async function createApplication(token) {
  ...
  try {
    ...
    if (res.status === 201 || res.status === 200) {
      ...
      console.log(
        "Management API requires authentication - skipping auto-creation",
      );
      console.log(
        "Please create the application manually via the Admin Console at http://localhost:3002",
      );
      return null;
    } else {
...
async function createUser(token) {
  ...
  try {
    ...
    if (res.status === 201 || res.status === 200) {
      ...
      console.log(
        "Please create the user manually via the Admin Console at http://localhost:3002",
      );
      return null;
    } else if (
      res.status === 422 &&
      res.data?.code === "user.username_already_in_use"
    ) {
...
async function seed() {
  ...
  try {
    ...
    console.log(
      "please configure the following manually via Admin Console (http://localhost:3002):",
    );
    console.log("");
